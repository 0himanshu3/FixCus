...
const Header = ({ notifications }) => {
  ...
  <>
    ...
    <div
      className={`fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity duration-300 ease-in-out ${menuModalOpen ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
      onClick={() => setMenuModalOpen(false)}
    ></div>

    <div
      className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-gradient-to-b from-purple-900 to-purple-800 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${menuModalOpen ? "translate-x-0" : "translate-x-full"
        }`}
    >
