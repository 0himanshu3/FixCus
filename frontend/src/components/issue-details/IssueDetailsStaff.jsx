...
export default function IssueDetailsStaff() {
  ...
  const [supervisorTasksSectionOpen, setSupervisorTasksSectionOpen] =
    useState(true); // optional toggle
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-pink-800 px-4 py-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {assignedRole && (
          <div className="bg-gradient-to-r from-green-400 to-green-500 rounded-xl p-4 shadow-lg border-4 border-purple-600">
            <p className="text-white font-black text-lg">
              🎭 You are assigned as {assignedRole}
            </p>
          </div>
        )}
          ...
          <div className="bg-gradient-to-r from-pink-300 to-pink-200 rounded-xl p-6 shadow-xl border-4 border-purple-600">
            ...
            {userTasks.length === 0 ? (
              <p className="text-purple-700 font-semibold">
                No tasks assigned to you for this issue.
              </p>
            ) : (
              userTasks.map((task) => {
                ...
                const isOverdue =
                  hasDeadlinePassed(task.deadline) && !isCompleted;

                return (
                  <div
                    key={task._id}
                    className="bg-white rounded-lg p-4 mb-4 shadow-md border-2 border-pink-400">
                    <div className="flex justify-between">
                      <div>
                        <h3 className="font-black text-xl text-purple-900 overflow-hidden">
                          {task.title}
                        </h3>
                    ...
                    {task.taskCompletionProof && (
                      <div className="mt-3 bg-pink-50 p-3 rounded-lg border-2 border-purple-300">
                        ...
                        <p className="text-xs text-purple-600 mt-1">
                          Proof Submitted:{" "}
                          {task.taskProofSubmitted ? "Yes" : "No"}
                        </p>
                      </div>
                    )}
                    ...
                    {task.taskUpdates?.length > 0 && (
                      <div className="mt-3">
                        <h4 className="font-black text-purple-900 overflow-hidden">
                          📋 Updates
                        </h4>
        ...
        {assignedRole === "Coordinator" && (
          <div className="bg-gradient-to-r from-pink-300 to-pink-200 rounded-xl p-6 shadow-xl border-4 border-purple-600">
            <h2 className="text-3xl font-black text-purple-900 mb-4 overflow-hidden">
              👷 Workers Assigned to This Issue
            </h2>
                  ...
                  <div
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (isAssigning || assignModalOpen || isResolved)
                            return;
                      ...
                      {workerTasks.length === 0 ? (
                        ...
                        workerTasks.map((task) => {
                          ...
                          return (
                            <div
                              key={task._id}
                              className="p-3 bg-pink-50 rounded-lg border-2 border-purple-300">
                              <div className="flex justify-between">
                                <div>
                              ...
                              {task.taskUpdates?.length > 0 && (
                                <div className="mt-2">
                                  <h4 className="font-bold text-purple-900 overflow-hidden">
                                    📋 Updates
                                  </h4>
        ...
        {assignedRole === "Supervisor" && (
          <div className="bg-gradient-to-r from-pink-300 to-pink-200 rounded-xl p-6 shadow-xl border-4 border-purple-600">
            <h2 className="text-3xl font-black text-purple-900 mb-4 overflow-hidden">
              🎭 Coordinators Assigned to This Issue
            </h2>
                    ...
                    <div className="mt-3 space-y-2">
                      {coordTasks.length === 0 ? (
                        ...
                        coordTasks.map((task) => {
                          ...
                          return (
                            <div
                              key={task._1d ? task._1d : task._id}
                              className="p-3 bg-pink-50 rounded-lg border-2 border-purple-300">
                              <div className="flex justify-between">
                                <div>
                                  <p className="font-bold text-purple-900">
            ...
            <h2 className="text-3xl font-black text-purple-900 mb-4 overflow-hidden">
              🧭 Tasks Escalated from Workers/Coordinators
            </h2>

            {/* Show tasks assigned to supervisors (flatten or use current user's tasks) */}
            {(issue?.staffsAssigned || []).filter(
              (s) => s.role === "Supervisor" && s.user
            ).length === 0 ? (
              <p className="text-purple-700 font-semibold">
                No supervisor tasks.
              </p>
              ...
              (issue?.staffsAssigned || [])
                .filter(
                  (s) => s.role === "Supervisor" && Array.isArray(s.tasks)
                )
                .flatMap((s) => s.tasks)
                .map((task, idx) => {
                  const isCompleted = task.status === "Completed";
                  ...
                  return (
                    <div
                      key={task._id || task.id || `sup-task-${idx}`}
                      className="bg-white rounded-lg p-4 mb-4 shadow-md border-2 border-pink-400">
                      <div className="flex justify-between">
                        <div>
                          <h3 className="font-black text-xl text-purple-900 overflow-hidden">
            ...
            <div className="mt-6 bg-white rounded-xl p-5 shadow-lg border-4 border-purple-500">
              <h3 className="font-black text-2xl text-purple-900 mb-3 overflow-hidden">
                🎊 Resolve Issue
              </h3>
              <p className="text-sm text-purple-700 mb-4 font-semibold">
                Previously you could enter a quick summary — now use the new
                button below to submit a full resolution report with images and
                ratings for staff.
              </p>
              <button
          ...
          {reassignModalOpen && reassignTask && (
            <motion.div
              ...
              <div className="bg-gradient-to-br from-pink-200 to-pink-300 p-6 rounded-2xl w-full max-w-md relative shadow-2xl border-4 border-purple-600">
                <button
                  className="absolute top-2 right-3 text-3xl font-black text-purple-900"
                  onClick={() => setReassignModalOpen(false)}>
                  &times;
                </button>
                <h3 className="text-2xl font-black text-purple-900 mb-3">
                  🔁 Reassign — {reassignTask.title}
                </h3>
                ...
                <select
                  ...
                  {(coordinatorsAssigned || []).map((c) => (
                    <option
                      key={c.user._id || c.user.id || c.user.email}
                      value={c.user._id || c.user.id || c.user.email}>
          ...
          {superCompleteModalOpen && superCompleteTask && (
            <motion.div
              ...
              <div className="bg-gradient-to-br from-pink-200 to-pink-300 p-6 rounded-2xl w-full max-w-md relative shadow-2xl border-4 border-purple-600">
                ...
                <h3 className="text-2xl font-black text-purple-900 mb-3">
                  ✅ Complete — {superCompleteTask.title}
                </h3>
                ...
                <input
                  ...
                  multiple
                  onChange={(e) =>
                    setSuperCompleteFiles(Array.from(e.target.files))
                  }
                ...
                <p className="text-xs text-purple-700 mt-3 font-semibold">
                  This will mark the task as Completed immediately (no approval
                  required).
                </p>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              <div className="bg-gradient-to-br from-pink-200 to-pink-300 p-6 rounded-2xl w-full max-w-md relative shadow-2xl border-4 border-purple-600">
                ...
                <h3 className="text-2xl font-black text-purple-900 mb-3 overflow-hidden">
                  📝 Add Update — {updateModalTask.title}
                </h3>
                <textarea
                  rows={4}
                  value={updateText}
          ...
          {proofModalOpen && proofModalTask && (
            <motion.div
              ...
              <div className="bg-gradient-to-br from-pink-200 to-pink-300 p-6 rounded-2xl w-full max-w-md relative shadow-2xl border-4 border-purple-600">
                ...
                <h3 className="text-2xl font-black text-purple-900 mb-3 overflow-hidden">
                  📸 Submit Proof — {proofModalTask.title}
                </h3>
                <textarea
                  rows={3}
                  value={proofText}
                ...
                <p className="text-xs text-purple-700 mt-3 font-semibold">
                  Images are uploaded to Firebase and URLs are sent to the
                  server.
                </p>
              </div>
            </motion.div>
          ...
          {assignModalOpen && assignModalAssignee && (
            <motion.div
              ...
              <div className="bg-gradient-to-br from-pink-200 to-pink-300 p-6 rounded-2xl w-full max-w-md relative shadow-2xl border-4 border-purple-600">
                ...
                <h3 className="text-2xl font-black text-purple-900 mb-3 overflow-hidden">
                  🎯 Assign Task to {assignModalAssignee.name} (
                  {assignModalAssignee.role})
                </h3>
                <input
                  type="text"
                  placeholder="Title"
              ...
              <div className="min-h-screen px-4 py-8 flex items-center justify-center">
                <div
                  ...
                  <input
                    ...
                    multiple
                    onChange={(e) =>
                      setResolveFiles(Array.from(e.target.files))
                    }
                  ...
                  <div className="mt-4">
                    <h4 className="font-black text-xl text-purple-900 mb-3 overflow-hidden">
                      ⭐ Rate staff performance
                    </h4>
                    <p className="text-xs text-purple-700 mb-3 font-semibold">
                      Provide a rating (1-5) and optional comment for each
                      worker/coordinator involved.
                    </p>
