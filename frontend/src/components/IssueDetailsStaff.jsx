...
export default function IssueDetailsStaff() {
  ...
  const uploadFilesToFirebase = async (files) => {
    ...
    setProofUploadProgress(progressArr);
    ...
    setTimeout(() => setProofUploadProgress([]), 600);
    return urls;
  };
  ...
  const submitTaskProof = async () => {
    ...
    try {
      const urls = await uploadFilesToFirebase(proofFiles);
      ...
      {assignedRole === "Supervisor" && (
        <div className="mt-6">
          ...
          <div className="mt-6 border p-4 rounded-md">
            ...
            <textarea className="w-full border p-2 rounded-md mb-2" placeholder="Summary of work done..." value={issueSummary} onChange={(e) => setIssueSummary(e.target.value)} />
            <button className="px-3 py-1 bg-green-500 text-white rounded-md" onClick={resolveIssue}>Submit Resolution</button>
          </div>
        </div>
      )}
